...
router.get('/', function (req, res, next) {
  res.render('index', {title: 'UriChen'});
});
...
router.get('/api/homeshoplist', (req, res) => {
  ...
  // 1.1 数据库查询的语句
  let sqlStr = 'SELECT * FROM pdd_home_shop LIMIT ' + (pageNo - 1) * pageSize + ',' + pageSize;
  // console.log(sqlStr);
...
router.post('/api/login_code', (req, res) => {
  ...
  // 2. 验证验证码是否正确
  if (users[phone] !== code) {
    res.json({err_code: 4, message: '验证码不正确!'});
  }
...
// 添加/删除购物车商品
router.post('/api/update_carts_goods', (req, res) => {
  console.log(req.body);
  // 1. 验证用户
  let user_id = req.body.user_id;
  if (!user_id || user_id !== req.session.userId) {
  ...
  let sqlStr = '';
  if (buy_count < 2 && !req.body.isAdd) {
    sqlStr = "DELETE FROM pdd_cart WHERE goods_id = '"+ goods_id +"' AND user_id = '"+ user_id +"'"
  } else {
  ...
  conn.query(sqlStr, (error, results, fields) => {
    if (error) {
      ...
      res.json({success_code: 200, message: '数据修改成功!'});
    }
  })
});
